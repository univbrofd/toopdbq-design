...
| key | mqmav1c7_wnam | mqmaxkwz_tkvb |
|---|---|---|
| `isVideo` | `true` | `false` |
| `quality` | `standard` | `standard` |
| `taskId` | `29e73014-85e2-45fe-844d-9fa1f2c6dab5` | `c9da877d-0144-42fd-96a8-51b53b2ef639` |
| `glbBytes` | `894008`（≈873 KB） | `745372`（≈728 KB） |
| `time` | `1781956532202`（ms） | `1781956666425`（ms） |
| `name` | `social_coou_…_0.mp4`（midjourney 動画） | `u5686972165_CANON_EOS_R5…_0.png`（midjourney 画像） |
...
## Claude Design に貼るプロンプト（今回追加分: mqmaxkwz_tkvb）

```
to claude
------------------------------------------------------
Toopdbq Studio が実生成した 3D セッション「mqmaxkwz_tkvb」（画像投稿 → そこから起こした GLB。動画なし）
を素材として渡す。まずこれらを fetch して使える状態にして（GLB はバイナリ添付できないので raw から取得）。
...
素材（public raw）:
- GLB:   https://raw.githubusercontent.com/univbrofd/toopdbq-design/main/assets/sample/3d/mqmaxkwz_tkvb/model.glb
- 画像:  https://raw.githubusercontent.com/univbrofd/toopdbq-design/main/assets/sample/3d/mqmaxkwz_tkvb/thumbnail.jpg

GLB は <model-viewer> で、画像は <img> で扱える。これは画像投稿なので動画は無い（画像 → 3D の 2 段）。
カタログには動画投稿の別セッション mqmav1c7_wnam も載っている。今は素材を取り込むところまででよく、
