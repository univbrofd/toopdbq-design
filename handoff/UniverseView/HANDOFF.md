...
`lib-design/DesignSystem/preview/` の specimen を**実機スクショに合わせて作り直す**（`_ds_manifest.json` の
...
- `lib-design/handoff/ThreeDTerrestrialView/HANDOFF.md` / `lib-design/handoff/TerrestrialPin/HANDOFF.md`

DS 基盤:
- `lib-design/DesignSystem/USAGE_RULES.md` / `taste.md` / `colors_and_type.css` / `preview/components.css`
- `lib-design/DesignSystem/_ds_manifest.json`（索引）

## Claude Design に貼るプロンプト

```
design/ds-sync の DS を正として、home 画面 UniverseView の specimen を「実機スクショに合わせて」直して。
現 comp-universe-screen.html は実機とかなり違うので、添付スクショ（real-*.jpg）に視覚を寄せること。
private repo なので GitHub コネクタで univbrofd/toopdbq の design/ds-sync を読む。
...
実機スクショ(正):
  https://raw.githubusercontent.com/univbrofd/toopdbq/design/ds-sync/lib-design/handoff/UniverseView/shots/real-home.jpg
  https://raw.githubusercontent.com/univbrofd/toopdbq/design/ds-sync/lib-design/handoff/UniverseView/shots/real-objects.jpg
  https://raw.githubusercontent.com/univbrofd/toopdbq/design/ds-sync/lib-design/handoff/UniverseView/shots/real-side-rail.jpg
  https://raw.githubusercontent.com/univbrofd/toopdbq/design/ds-sync/lib-design/handoff/UniverseView/shots/real-reel.jpg
...
`references/handoff.md` C の reconcile（更新カードを repo へ、manifest を superset マージ）→ `design/ds-sync` へ push。
