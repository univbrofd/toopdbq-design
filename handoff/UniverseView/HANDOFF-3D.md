# HANDOFF-3D（アーカイブ）— UniverseView：クラスタ ⇄ リール ⇄ メイン ⇄ フォーカス の連携リファクタ

> **アーカイブ**: 3D 表示デザインの探求記録。アプリ本体は 3D 表示・3D 投稿レーンを撤去済みで、
> このデザインは現在の実装対象ではない。specimen（`UniverseView.html` ＋ `earth/*.js`）と本書・`USE_CASES.md` は
> デザイン資産として温存する（変更・削除しない）。現行タスクは `HANDOFF.md`。
