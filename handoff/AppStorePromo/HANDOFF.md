...
- 440×956。**地は黒**、下半分（45%〜）に DS の `--gradient-colorful-linear` を逆順（要素を 180° 回転 = 315°・斜めに変化）を opacity .78 のマスクで透過表示（濃いめ）。文字は白。書き出し 1320×2868 @3x（`node scripts/design/poster_export.mjs`）
...
- accent = 1 面 1 語。色は面ごとにグラデーションの 4 色 `--grad-pink → --grad-teal → --grad-yellow → --grad-golden`（`data-accent`）
