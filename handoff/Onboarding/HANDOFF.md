...
- repo: `univbrofd/toopdbq`（private, GitHub コネクタで読む）
- branch: `design/onboarding-ip17`
- raw base: `https://raw.githubusercontent.com/univbrofd/toopdbq/design/onboarding-ip17`
...
## 実装 = 正（参照ファイル, すべて raw base からの相対）
...
現状の specimen（移植元。iPhone 17 枠へ作り直す）:
- `/lib-design/handoff/Onboarding/comp-onboarding-01..06-*.html` と `onboarding.css` / `card.css` / `components.css`

トークン根拠（新規発明禁止・これを使う）:
- `/lib-design/DesignSystem/colors_and_type.css`（役割トークン・`--gradient-colorful` 等）
- `/lib/component/ui/resource/FigmaColors/FigmaColors.dart`（`colorfulRadialGradient` / `colorfulLinearGradient`）
- 索引: `/lib-design/DesignSystem/_ds_manifest.json`

現状の比較プレビュー（差分の確認用）:
- `/lib-design/preview/Onboarding-01..06.png`（左 design ｜ 右 Flutter 実機、両者 iPhone 17 枠）
