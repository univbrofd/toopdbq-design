...
- **`handoff/PlanetView/earth-globe.html`** — live specimen（本体エンジンをそのまま読む）。
- **`handoff/PlanetView/earth/`** — 本体 `assets/web/earth/` の 5 モジュールのコピー
  （`bridge.js` / `map-core.js` / `pins.js` / `overlay.js` / `models.js`）。`bridge.js` は
  `FlutterChannel` 不在で `_sendToHost` が no-op になり、Flutter なしで起動する。
- サンプル 3D は共有プール `assets/sample/models/model-0N.glb` を参照。
