...
## クエストヘッダー（`WdQuestHeader`）— 編集 / 確認のみ
...
- **撮影中は中央を出さない**（`showQuest: false` = 戻るボタンだけ）。ファインダーを文字で塞がない

## 状態 1 — 撮影（`shooting`）

- ヘッダーは**戻るボタンだけ**（お題 / カウントダウンは出さない）
...
- テキスト入力は**自動で開かない**（撮れた画をまず確認させる）。プレースホルダ「テキストを入力」を置き、Tt で入力を開く
