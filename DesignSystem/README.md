...
**Backgrounds.** Full-bleed photography is the backdrop for nearly every screen — candid smartphone shots, warm-meets-cool, soft grain. Sample imagery comes exclusively from the repo pool (v2): `assets/sample/background/` (16 full-bleed 816×1456 backgrounds for splash / login backing / story full-bleed / hero), `assets/sample/reel/` (15 vertical post/reel photos for 9:16 cells, pin thumbs and circle covers) and `assets/sample/user/` (16 avatar squares, face-centered via `object-position`) — see `preview/brand-imagery.html` for the labelled catalog; external CDN references are banned. The app logo is `logo_toopdbq.png` (transparent PNG — use as-is, never redraw). The auth screen uses `auth_background.png`, a grainy blue→orange gradient figure. Behind the globe: deep space / star field (`assets/celestial/stars`). No flat color screens except loaders/dialogs (which flip to a light scrim).
